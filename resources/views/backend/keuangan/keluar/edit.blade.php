...
<div class="row">
	<form class="col" method="post" action="{{route('keluar.update', $keluar->id)}}">
	@csrf
	@method('PATCH')
	<div class="container">
	<div class="form-group">
		<label>Jumlah (Rp)</label>
		<input type="number" class="form-control @error('jumlah') is-invalid @enderror" value="{{$keluar->jumlah}}" name="jumlah">
		@error('jumlah')
	...
	<div class="form-group">
		...
		<input type="text" class="form-control @error('keterangan') is-invalid @enderror" value="{{$keluar->keterangan}}" name="keterangan">
		@error('keterangan')
            <span class="invalid-feedback" role="alert">
            	<strong>{{ $message }}</strong>
            </span>
